...
但是“预编译成 `.js`”并不等于“自动兼容所有低版本 Vue”。它只能降低一部分编译链差异问题。
...
这里要注意：是否可运行还取决于运行时依赖是否存在、helper 是否兼容、以及产物目标格式是否匹配。例如某些新宏会编译到新运行时 helper，低版本 `vue` 运行时不一定具备。
...
在打包 `Vue` 组件库时，请使用 [Vite 的库模式](https://vite.dev/guide/build.html#library-mode) 来进行构建，因为这是官方维护的推荐方式。
