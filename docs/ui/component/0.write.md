...
1. **基础功能**
   - 支持不同状态（正常、悬浮 `hover`、聚焦 `focus-visible`、按下 `active`、禁用 `disabled`）。
...
5. **进阶：可访问性（a11y）**
   - 原生 `<button>` 默认已经有按钮语义，不需要额外加 `role="button"`。
   - 如果渲染成 `<a>`、`div` 等非按钮元素，才需要补足角色、键盘触发和禁用语义。
   - 加载态可以用 `aria-busy` 表达当前操作还没完成。
...
在完成组件开发之后，下一步是进入组件构建，把这个组件变成稳定的可分发产物。
