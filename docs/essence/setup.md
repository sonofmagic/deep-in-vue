...
> `<script setup>` 是 Vue 3 提供的一种 **编译时语法糖**，它让你以更简洁的方式使用 Composition API，本质上会在**构建阶段**被 Vue 编译成普通的 `setup()` 相关代码。

换句话说：它不是普通 JavaScript 模块会原样执行的结构，而是 Vue SFC 编译器预处理前的源码形态。

比如 TypeScript 类型在普通 JavaScript 产物中会被擦除，但 `defineProps<Props>()` 里的部分类型结构，可能会被 Vue 编译器提取成运行时 `props` 选项。
...
| 功能            | 实现机制                                                                                                                            |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 编译时语法糖    | 构建时由 Vue 编译器解析（通常由 Vite / Webpack 插件接入）                                                                           |
| 自动导出到模板  | 所有顶层变量都默认暴露给 `<template>`                                                                                               |
| 更好的类型推导  | 对 TypeScript 支持更自然，部分编译宏能读取类型 AST 并生成对应运行时代码                                                             |
...
> `<script setup>` 是 Vue 3 的编译时语法糖，最终会被编译进组件的 `setup()` 逻辑，并和模板生成的 `render()` 连接起来。它让 Composition API 写法更自然、简洁、类型友好，是 Vue 3 推荐的组件书写方式。
>
