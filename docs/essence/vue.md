...
Vue 团队正在开发 Vapor Mode，这是一种新的编译策略。在 Vapor Mode 下，`.vue` 文件的模板不再编译为虚拟 DOM 的渲染函数，而是编译为更高效的**直接 DOM 操作**代码，类似 Svelte 和 Solid.js 的编译方式。

这意味着 `.vue` 文件的本质不会改变（依然是模板 + 脚本 + 样式的组合），但编译产物会有根本性的变化——从虚拟 DOM diff 变为精确的 DOM 更新指令，带来更好的运行时性能和更小的包体积。
